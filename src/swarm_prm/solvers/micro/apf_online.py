"""
    APF with online goal assignment
"""
...
from swarm_prm.solvers.macro.teg.gaussian_utils import GaussianNode
from swarm_prm.solvers.micro.spatial_hash import SpatialHash

class APFOnlineSolver:
    """
    ...
    def initailize_starts(self):
        ...
        for agent_idx in range(self.num_agent):
            ...
            while init_attempt < self.max_init_attempt:
                ...
                pt = self.macro_solution[agent_idx][0] \
    ...
    def update(self, timestep):
        ...
        """
        order = [i for i in range(len(self.macro_solution))]
        if self.ordering_strategy == "SEQUENTIAL": # Nothing needed to do
            ...
            dist = [np.linalg.norm(self.macro_solution[agent_idx][timestep] - self.solution_trajectory[agent_idx][-1])
                    for agent_idx in range(self.num_agent)]
