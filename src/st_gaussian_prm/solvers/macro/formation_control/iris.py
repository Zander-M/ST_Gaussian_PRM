...
class IrisSampler:
    ...
    def add_polygon_as_box(self, polygon, buffer=0.01):
        """
    ...
    def iris_hpoly_to_polygon(self, hpoly: HPolyhedron):
        """
    ...
    def shapely_polygon_to_hpoly(self, polygon):
        """
    ...
    def run_iris_with_shapely(self, seed_point, obstacles, bounds=[[-100, 100], [-100, 100]]):
        ...
        for poly in obstacles:
            if isinstance(poly, Polygon):
                geom_id = self.shapely_polygon_to_vpolytope(poly)
                geom_ids.append(geom_id)
            else:
                geom_id = self.add_polygon_as_box(poly)
                geom_ids.append(geom_id)
    ...
    def plot_region_and_obstacles(self, region_poly, obstacles, seed=None):
        """
...
if __name__ == "__main__":
    ...
    seed = [2, 2]
    sampler = IrisSampler(None)
    iris_poly = sampler.run_iris_with_shapely(seed, obstacles)
    sampler.plot_region_and_obstacles(iris_poly, obstacles, seed)
